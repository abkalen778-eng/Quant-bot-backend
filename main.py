...
app = FastAPI(title="Quant Bot Backend", version="3.0.0", description="Coinbase confirmed-breakout paper scanner with guarded algorithmic execution controls.")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
DEFAULT_HEADERS = {"User-Agent": "quant-bot-backend/3.0"}
...
def _execute(product: str, side: str, size_usd: float, require_signal: bool = True) -> dict[str, Any]:
    ...
    preview = {
        ...
        "estimated_base_size": round(base, 12),
        "reference_price": px,
        "signal": sig["signal"],
...
def root() -> dict[str, Any]:
    return {"name":"Quant Bot Backend","version":"3.0.0","strategy":STRATEGY_NAME,"status":"online","auto_trading":AUTO_TRADING,"trading_enabled":TRADING_ENABLED,"dry_run":DRY_RUN,"kill_switch":KILL_SWITCH,"products":AUTO_PRODUCTS,"scan_interval_seconds":SCAN_INTERVAL_SECONDS,"max_order_usd":MAX_ORDER_USD,"auto_order_usd":AUTO_ORDER_USD}
...
def algo_status() -> dict[str, Any]:
    with state_lock:
        return {**bot_state,"configured":AUTO_TRADING,"dry_run":DRY_RUN,"trading_enabled":TRADING_ENABLED,"kill_switch":KILL_SWITCH,"products":AUTO_PRODUCTS,"interval_seconds":SCAN_INTERVAL_SECONDS,"strategy":STRATEGY_NAME}
