...
def test(daily,hourly,start,volume_min=2.0,consolidation_max=.08):
 returns=[];log=[];busy=0.
 for i in range(40,len(daily)-1):
  ...
  if vr<volume_min or cons>consolidation_max or not near or bp<.01:continue
...
def run():
 now=datetime.now(timezone.utc).replace(second=0,microsecond=0);start=now-timedelta(days=DAYS);results=[];strict_all=[];relaxed_all=[]
 for p in PRODUCTS:
  try:
   daily=fetch(p,86400,start-timedelta(days=60),now);hourly=fetch(p,3600,start-timedelta(days=2),now)
   sm,st=test(daily,hourly,start.timestamp(),2.0,.08)
   rm,rt=test(daily,hourly,start.timestamp(),1.5,.10)
   results.append({"product":p,"strict":sm,"relaxed":rm,"strict_trade_log":st,"relaxed_trade_log":rt})
   strict_all.extend([x["net_return_pct"]/100 for x in st]);relaxed_all.extend([x["net_return_pct"]/100 for x in rt])
  except Exception as e:results.append({"product":p,"error":f"{type(e).__name__}: {e}"})
 return {
  "period_days":DAYS,
  "approx_years":3,
  "products":PRODUCTS,
  "strict":{"filters":{"volume_min":2.0,"consolidation_max_pct":8.0},"aggregate_trade_metrics":metrics(strict_all)},
  "relaxed":{"filters":{"volume_min":1.5,"consolidation_max_pct":10.0},"aggregate_trade_metrics":metrics(relaxed_all)},
  "unchanged_filters":{"near_resistance_max_pct":8.0,"breakout_confirm_pct":1.0,"risk_per_trade_pct":.5,"fee_bps_per_side":40.,"slippage_bps_per_side":10.},
  "results":results,
  "strategy":"confirmed_breakout_strict_vs_relaxed_3y"
 }
state={"status":"not_started","result":None,"error":None}
def worker():
 state["status"]="running"
 try:r=run();state["result"]=r;state["status"]="complete";print("CONFIRMED_BREAKOUT_FILTER_COMPARISON="+json.dumps(r),flush=True)
 except Exception as e:state["status"]="failed";state["error"]=f"{type(e).__name__}: {e}";print("CONFIRMED_BREAKOUT_FILTER_COMPARISON_ERROR="+state["error"],flush=True)
@app.on_event("startup")
def startup():threading.Thread(target=worker,daemon=True,name="confirmed-breakout-filter-comparison").start()
